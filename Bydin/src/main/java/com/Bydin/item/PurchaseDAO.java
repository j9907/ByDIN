...
public interface PurchaseDAO {
	...
	@Select("select DISTINCT totalgoods.image, purchase_detail.item_name, purchase.totalcount, purchase.order_state,"
			+ "purchase_detail.item_price, purchase.totalprice, purchase.item_idx, purchase.member_idx " + 
	...
	@Update("update purchase_detail set order_state='구매완료', order_name=#{1.order_name}, order_postcode=#{1.order_postcode}, "
			+ "order_address=#{1.order_address}, order_detailAddress=#{1.order_detailAddress}, order_extraAddress=#{1.order_extraAddress}, "
			+ "order_req=#{1.order_req} where member_idx=#{0}")
